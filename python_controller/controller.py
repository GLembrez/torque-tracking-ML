...
class Controller:
    def __init__(self, router, router_real_time):
        ...
        # Magnetic constant of actuators
        self.kt = [0.11,0.11,0.11,0.11,0.076,0.076,0.076]
        # Gear ratio of harmonic drive
        self.gr = 100
        # list of the joint limits
        self.q_inf = np.array([-np.pi,-np.pi/2,-np.pi,-np.pi/2,-np.pi,-2,-np.pi])
        self.q_sup = np.array([np.pi,np.pi/2,np.pi,np.pi/2,np.pi,2,np.pi])
        # index of torque controlled actuators
    ...
    def InitCyclic(self, sampling_time_cyclic, t_end, print_stats):
        ...
        if base_feedback:
            ...
            # To ensure continuity, motor current is set to measure
            for x in self.controlled_index :
    ...
    def RunCyclic(self, t_sample, print_stats):
        ...
        while not self.kill_the_thread:
            ...
            if (t_now - t_cyclic) >= t_sample:
                ...
                for x in self.controlled_index:
                    # disable position tracking error
                    self.base_command.actuators[x].position = self.base_feedback.actuators[x].position
                    # First actuator torque command is set to last actuator torque measure times an amplification
                    self.base_command.actuators[x].current_motor = 3 / (self.gr * self.kt[i])
...
def main():
    ...
    with utilities.DeviceConnection.createTcpConnection(args) as router:
        ...
        with utilities.DeviceConnection.createUdpConnection(args) as router_real_time:
            ...
            example = Controller(router, router_real_time)
