...
input  - last dimension of the input tensor def to the LSTM model
hidden - last dimension of the hidden layer of the LSTM
output - dimension of the output tensor
layers - number of LSTM layers stackd on top one another
"""
...
def main():
    ...
    ap.add_argument("-x", "--xml", required=True, default=False)
    ...
    ap.add_argument("--train_steps", required=False, default=3600*1000, type=int)
    ap.add_argument("--valid_steps", required=False, default= 600*1000, type=int)
    ap.add_argument("--rate", required=False, default=1e-4, type=float)
    ap.add_argument("--tol", required=False, default=1e-2, type=float)
    args = ap.parse_args()


    # create mujoco simulations
    sim1 = Simulation(args.xml)
    sim2 = Simulation(args.xml)
    # generate training and validation datasets
    df_train = sim1.simulate(args.train_steps)
    df_valid = sim2.simulate(args.valid_steps)
    ...
    train_set = TorqueTrackingDataset(args.sequence, df_train)
    ...
    valid_set = TorqueTrackingDataset(args.sequence, df_valid)
    ...
    with torch.autograd.set_detect_anomaly(True):
        ...
        try:
            for epoch in range(args.epochs):
                ...
                if torch.abs(valid_loss_new-valid_loss)<args.tol:
